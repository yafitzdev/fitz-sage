...
from fitz_sage.cli.ui import RICH, Table, console, ui
...
def governance_stats(
    ...
    """
    from fitz_sage.evaluation import GovernanceStats
...
def _get_engine(collection: str, engine_name: str | None = None):
    ...
    from fitz_sage.runtime import create_engine, get_default_engine
...
def beir_benchmark(
    ...
    """
    try:
        from fitz_sage.evaluation.benchmarks.beir import BEIRBenchmark
    except ImportError:
        ui.error("BEIR benchmark requires: pip install fitz-sage[benchmarks]")
        raise typer.Exit(1)
...
def rgb_benchmark(
    ...
    """
    from fitz_sage.evaluation.benchmarks.rgb import RGBEvaluator, RGBTestType
...
def fitz_gov_benchmark(
    ...
    """
    from fitz_sage.evaluation.benchmarks.fitz_gov import FitzGovBenchmark, FitzGovCategory
...
def _display_fitz_gov_rich(result) -> None:
    ...
    from fitz_sage.evaluation.benchmarks.fitz_gov import FitzGovCategory
...
def run_all_benchmarks(
    ...
    if not skip_beir:
        try:
            ...
            ui.warning("Skipping BEIR (requires: pip install fitz-sage[benchmarks])")
    else:
        ui.info("Skipping BEIR benchmark")

    # fitz-gov (no external deps)
    ui.info("Running fitz-gov benchmark...")
    from fitz_sage.evaluation.benchmarks.fitz_gov import FitzGovBenchmark
