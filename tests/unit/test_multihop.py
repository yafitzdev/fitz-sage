...
class TestMultihopConfig:
    """Tests for MultihopConfig in schema."""

    def test_default_config(self):
        """Test default multi-hop configuration."""
        from fitz_ai.engines.fitz_rag.config.schema import MultihopConfig
    ...
    def test_max_hops_validation(self):
        """Test max_hops validation boundaries."""
        ...
        with pytest.raises(ValidationError):
            MultihopConfig(max_hops=6)

    def test_fitz_rag_config_includes_multihop(self):
        """Test that FitzRagConfig includes multihop."""
        from fitz_ai.engines.fitz_rag.config.schema import FitzRagConfig

        config = FitzRagConfig.from_dict(
            {
                "chat": {"plugin_name": "openai", "kwargs": {}},
        ...
        assert config.multihop.max_hops == 3
