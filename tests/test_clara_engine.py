...
class TestClaraEngine:
    ...
    def test_answer_success(self, mock_engine):
        ...
        # Mock retrieval
        mock_engine._retrieve = MagicMock(return_value=(
            ["doc_1", "doc_2"],
            [0.9, 0.8]
        ))

        # Mock generation
        mock_engine._generate = MagicMock(return_value=(
    ...
    def test_answer_respects_constraints(self, mock_engine):
        ...
        query = Query(
            text="Question?",
            constraints=Constraints(max_sources=3)
        )
        answer = mock_engine.answer(query)
