...
**Status**: Two-stage classifier with full feature parity. model_v8: 82.06% raw, calibrated 78.92% accuracy, min recall 77.9%. All 51 features available in production (ctx_* ported to feature_extractor.py). Proposals 1b+2 complete. 47 errors remain (23 trustworthy->disputed). Next: production integration (GovernanceDecider).
...
| 2026-02-09 | Dead code cleanup executed: removed 18 dead features, 2 unused plugins (deterministic_conflict.py, governance_analyzer.py), 3 dead factory functions. 826 lines deleted. Retrained on 29 clean features: model_v6 80.72% (identical to pre-cleanup). No regression. Committed as 59a1597. |
| 2026-02-09 | **Proposal 1: CA quality features — FAILED**. Implemented scoring prompt (SCORE: 0-10 + TYPE classification) in conflict_aware.py. Testing with qwen2.5:3b revealed fatal flaw: fast LLM **always returns SCORE: 10** regardless of content. Compatible texts (Paris capital facts) get SCORE: 10 TYPE: framing. Contradicting texts get SCORE: 10 TYPE: opposing. The 3B model cannot handle 0-10 scaling. Simpler type-only classification also failed (1/4 correct). All changes reverted. The "zero additional LLM cost" assumption in the proposal is invalid — the fast LLM lacks the nuance for scoring or type classification. |
