...
**Status**: Two-stage classifier with inter-chunk features. model_v7: Stage 2 CV 84.8%, calibrated 78.92% accuracy, min recall 77.9%. LLM scoring failed (3B can't score), deterministic text features succeeded (+10.5pp Stage 2 CV). 47 errors remain (23 trustworthy→disputed). Next: production integration (GovernanceDecider).
