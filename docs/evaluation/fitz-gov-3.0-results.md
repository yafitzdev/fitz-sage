...
### Feature Profile by Stage

**Stage 1** (abstain vs answerable) top features: `mean_vector_score`, `ca_fired`, `query_word_count`, `has_disputed_signal`, `vocab_overlap_ratio`

**Stage 2** (trustworthy vs disputed) top features: `ctx_length_mean`, `ctx_total_chars`, `ctx_length_std`, `ctx_mean_pairwise_sim`, `score_spread`, `chunk_length_cv`
...
The final +12-18pp improvement came from fixing a train/eval distribution mismatch: `extract_features.py` never computed embeddings, so `mean_vector_score=0` for all training cases.
...
fitz-gov 3.0 achieved **90.9% governance accuracy** on 1113 cases — the strongest result in the project's history. The key innovations were:

1. **3-class taxonomy** — removing the inseparable confident/qualified distinction
2. **Two-stage binary classifiers** — decomposing into cleaner sub-problems
3. **Threshold tuning for safety** — minimizing false-trustworthy (15 critical cases)
4. **Feature distribution fix** — ensuring training features match production

The 15 remaining critical cases are all hard-difficulty boundary cases that require constraint-level improvements (entity-mismatch detection, chunk-sufficiency checking) — the classifier has reached the ceiling of what current features can provide.
