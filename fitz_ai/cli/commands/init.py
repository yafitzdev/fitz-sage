...
def command(
    ...
    # If user selected a non-collection engine, skip plugin config
    caps = registry.get_capabilities(default_engine)
    if not caps.supports_collections:
        # For engines that don't need shared plugin config, copy their default.yaml
        global_config_yaml = _generate_global_config(default_engine)

        # Get engine-specific config from registry (auto-discovery)
