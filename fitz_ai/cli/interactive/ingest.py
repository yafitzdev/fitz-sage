...
"""
Ingest command: Ingest documents into vector database.

Usage:
    fitz ingest run ./documents collection
    fitz ingest run ./documents collection --chunker pdf_sections
    fitz ingest run ./documents collection --embedding openai
    fitz ingest run ./documents collection -q
"""
...
def _embed_with_progress(
    ...
    Args:
        embed_plugin: Embedding plugin instance with embed() method
    ...
    if RICH_AVAILABLE and show_progress and len(chunks) > 1:
        ...
        for i, chunk in enumerate(chunks):
            vector = embed_plugin.embed(chunk.content)
            vectors.append(vector)
            if not RICH_AVAILABLE and show_progress and (i + 1) % 10 == 0:
...
def command(
    ...
    Examples:
        fitz ingest run ./docs default
        fitz ingest run ./docs my_knowledge
        fitz ingest run ./docs my_docs --embedding openai
        fitz ingest run ./docs my_docs -q
        fitz ingest run ./doc.pdf papers --chunker pdf_sections
        fitz ingest run ./doc.pdf papers --chunker simple --chunk-size 500
    """
    ...
    # Use LLM plugin registry to get embedding plugin
