...
class VectorSearchStep(RetrievalStep):
    ...
    def execute(self, query: str, chunks: list[Chunk]) -> list[Chunk]:
        """
        Execute vector search with automatic multi-query expansion.

        For short queries (< min_query_length): single vector search.
        For long queries (with chat available): LLM extracts search terms,
        runs multiple searches, combines and deduplicates results.

        Pre-existing chunks (e.g., artifacts) are preserved and prepended.
        """
        # Decide: single search or multi-query expansion
        use_multi_query = (
