...
def command(
    ...
    config_path = fitz_dir / "config.yaml"
    
    # Also save to the engine config location
    engine_config_dir = Path.cwd() / "fitz" / "engines" / "classic_rag" / "config"
    
    if not non_interactive:
        if config_path.exists():
            if not prompt_confirm(f"Overwrite existing {config_path}?", default=False):
                print("Aborted.")
    ...
    # Save to .fitz/config.yaml
    config_path.write_text(config)
    print(f"✓ Saved to {config_path}")
    
    # Also update default.yaml if it exists
    if engine_config_dir.exists():
        default_yaml = engine_config_dir / "default.yaml"
        default_yaml.write_text(config)
        print(f"✓ Updated {default_yaml}")
