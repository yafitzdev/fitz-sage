...
fitzgov_dir = Path(__file__).resolve().parent.parent.parent.parent / "fitz-gov" / "data" / "tier1_core"
...
if len(s1_fn) > 0:
    print(f"  P(answerable) stats: mean={s1_fn['p_answerable'].mean():.3f}, median={s1_fn['p_answerable'].median():.3f}")
...
if len(s1_fp) > 0:
    print(f"  P(answerable) stats: mean={s1_fp['p_answerable'].mean():.3f}, median={s1_fp['p_answerable'].median():.3f}")
...
if len(disputed_missed) > 0:
    print(f"  P(trustworthy) stats: mean={disputed_missed['p_trustworthy'].mean():.3f}, median={disputed_missed['p_trustworthy'].median():.3f}")
...
if len(trust_as_disputed) > 0:
    print(f"  P(trustworthy) stats: mean={trust_as_disputed['p_trustworthy'].mean():.3f}, median={trust_as_disputed['p_trustworthy'].median():.3f}")
...
key_features = [
    'ca_fired', 'ca_signal', 'ie_fired', 'ie_signal', 'mean_vector_score',
    'score_spread', 'vocab_overlap_ratio', 'num_constraints_fired',
    'av_jury_votes_no', 'ca_pairs_checked', 'query_word_count',
    'num_chunks', 'ctx_length_mean', 'ctx_max_pairwise_sim',
    'ctx_mean_pairwise_sim', 'ctx_contradiction_count', 'ctx_number_variance',
]
...
for name, imp in sorted(
    zip(feature_cols, s1_model.feature_importances_), key=lambda x: -x[1]
)[:15]:
    print(f"  {name:35s}  {imp:.4f}")

print("\nStage 2 (trustworthy vs disputed):")
for name, imp in sorted(
    zip(feature_cols, s2_model.feature_importances_), key=lambda x: -x[1]
)[:15]:
    print(f"  {name:35s}  {imp:.4f}")
